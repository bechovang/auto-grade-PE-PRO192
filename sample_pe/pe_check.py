...
class PETestRunner:
    ...
    def compare_outputs(self, actual: str, expected: str, remove_spaces: bool = False, case_sensitive: bool = True) -> bool:
    ...
    def run_question(self, q_num: int, test_cases: list[dict]) -> dict | None:
        ...
        for tc in test_cases:
            ...
            if returncode != 0:
                ...
                print(f"└{'─' * 65}\n")
                results.append({"tc_num": tc_num, "passed": False, "max_mark": tc["mark"], "earned": 0.0})
                total_mark += tc["mark"]
                continue
            actual_display = stdout[:80] + "..." if len(stdout) > 80 else stdout
            print(f"│ 📤 Actual:   {actual_display}")
            passed = self.compare_outputs(stdout, tc["expected_output"], tc["remove_spaces"], tc["case_sensitive"])
            if passed:
                print("│ ✅ PASS")
                print(f"│ 💯 Score: {tc['mark']}/{tc['mark']}")
                earned_mark += tc["mark"]
                results.append({"tc_num": tc_num, "passed": True, "max_mark": tc["mark"], "earned": tc["mark"]})
            else:
                print("│ ❌ FAIL")
                print(f"│ 💯 Score: 0/{tc['mark']}")
                results.append({"tc_num": tc_num, "passed": False, "max_mark": tc["mark"], "earned": 0.0})
        ...
        return {"question": q_num, "total_mark": total_mark, "earned_mark": earned_mark, "results": results}
    ...
    def print_summary(self, all_results: list[dict]) -> None:
        ...
        for result in all_results:
            ...
            for tc_result in result["results"]:
                ...
                print(f"   {tc_status} TC{tc_result['tc_num']}: {tc_result['earned']:.1f}/{tc_result['max_mark']:.1f}")
        print("─" * 70)
